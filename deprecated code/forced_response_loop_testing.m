function forced_response_loop_testing(kin,gamma_in)
% function forced_response_loop_testing(kin,gamma_in)
% another iteration of the forced response loop. not used in final product.
...
k = kin(k_idx);
    for g_idx=1:length(gamma_in)
        ...
        fname = sprintf('./data/forced_response_sweep/forced_response_data_testing-k-%d-gamma-%.1f.mat',k,gamma_in(g_idx));
