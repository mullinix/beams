function myBeamForced2D()

%% Setup mass and stiffness element matrices
L = 1.0; % length of beam [m] (0.45)
N = 25; % number of elements [1]
...
for i=1:N;%
	j = 2*i-1;
	k = j+3;
	MM1(j:k,j:k) = MM1(j:k,j:k) + M;
	KK1(j:k,j:k) = KK1(j:k,j:k) + K;
end

MM = zeros(4*(N+1)); 
KK = MM;
% TODO: each direction shall have its own MMi, KKi, where there are
% different moments of inertia Ii
MM = [MM1,zeros(2*(N+1));...
      zeros(2*(N+1)),MM1];
KK = [KK1,zeros(2*(N+1));...
      zeros(2*(N+1)),KK1];
...
space_rescaling_factor = 10^(-KK_min);% alpha
time_rescaling_factor = 10^(KK_min-MM_min);% beta
...
%% Apply BC's - fixed -> free
KK([1:2,2*(N+1)+1:2*(N+2)],:) = []; KK(:,[1:2,2*(N+1)+1:2*(N+2)]) = [];
MM([1:2,2*(N+1)+1:2*(N+2)],:) = []; MM(:,[1:2,2*(N+1)+1:2*(N+2)]) = [];
N=N-1;
...
y0 = [shape(:,mode_to_try)./max(abs(X));zeros(4*(N+1),1)];%
X = X./max(abs(X));
X = [X(1:N+1),X(N+2:end)];
% MM_inv = inv(MM);
...
MM_big = [eye(size(MM)),zeros(size(MM));zeros(size(MM)),MM];
% options = odeset('Mass',MM_big,'RelTol',1e-6);
options = odeset('RelTol',1e-5,'Mass',MM_big, ...
...
w=y(:,end/4+1:2:end/2);
y_out_length = cols/8;
BAR = linspace(0,L,y_out_length); %#ok<*NASGU>

%% Calculate SSE, print to command window
err_vec = y(end,1:2:cols/2-1)'-[X(:,1);X(:,2)];
...
if(wiggleBar)
    ...
    h1 = plot3(BAR,X(:,1),X(:,2));
    hold on;
    h2 = plot3(BAR,v(plot_idx,:),w(plot_idx,:),'g');
    % % set data sources for plots
    set(h1,'YDataSource','X');
    set(h1,'XDataSource','BAR');
    set(h2,'YDataSource','vv');
    set(h2,'ZDataSource','ww');
    ...
    for plot_idx = 2*skip_pts:skip_pts:length(t)
        vv=v(plot_idx,:)';
        ww=w(plot_idx,:)';
        set(h2,'XData',BAR,'YData',vv,'ZData',ww);
        title(sprintf('idx: %d t: %e',plot_idx,t(plot_idx)));
        pause(0.01);%
    end
end
